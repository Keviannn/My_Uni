...
//Version 3.1:
//- Use of calloc() to avoid garbage in new Subject pointers
//- Correction of double error msg
...
{
    Subject *temp = (Subject *)calloc(1, sizeof(Subject));
...
void new_exam(Subject * temp, int i)
{
    ...
    if (input[0] == 'y')
    {
        ...
        while (!k)
        {
            ...
            //TODO: i can check if input[1] == \n and create a new error msg from that
...
void program()
{
    ...
    while (!k)
    {
        ...
        {
        case 1:
            //TODO: here wait_enter() works differently
            new_subject();
            wait_enter();
            break;
...
int main(void)
{
    ...
    //TODO: implementation of this code in a function enables subject names edition
    /*char name[31];
    Subject *temp = (Subject *)calloc(1, sizeof(Subject));
    FILE *file = fopen(get_name(name), "r+");
    fread(temp, sizeof(Subject), 1, file);
    clean_stdin();
    printf("name");
    scanf("%30[^\n]", temp->name);
